...
const Oeffnungszeiten = () => {
	// Original calendar state preserved for future use:
	// const [month, setMonth] = useState(new Date());
	// const modifiers = useMemo(() => {
	//   const start = startOfMonth(addMonths(month, -1));
	//   const end = endOfMonth(addMonths(month, 1));
	//   const days = eachDayOfInterval({ start, end });
	//   const hauptsaison: Date[] = [];
	//   const uebergang: Date[] = [];
	//   const winterpause: Date[] = [];
	//   days.forEach((day) => {
	//     const season = getSeason(day.getMonth());
	//     if (season === "hauptsaison") hauptsaison.push(day);
	//     else if (season === "uebergang") uebergang.push(day);
	//     else winterpause.push(day);
	//   });
	//   return { hauptsaison, uebergang, winterpause };
	// }, [month]);
					...
					<div className="container mx-auto px-4 max-w-3xl">
						<div className="text-center mb-10">
							...
							<p className="text-muted-foreground">
								Regelungen an gesetzlichen Feiertagen in
								Baden-Württemberg
							</p>
						</div>
						<div className="bg-card rounded-xl border border-border overflow-hidden">
							{holidays.map((h, i) => (
								...
								>
									...
									<span
										className={`text-sm ${h.status.includes("Geschlossen") ? "text-destructive" : "text-primary"} font-medium`}
									>
										{h.status}
									</span>
								</div>
						...
						<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
							<Card>
								<CardContent className="pt-6 pb-6 px-6">
									...
									<p className="text-muted-foreground text-sm">
										Bei leichtem Regen sind wir geöffnet –
										das gehört zum Abenteuer! Bei Gewitter,
										Sturm oder starkem Regen schließen wir
										aus Sicherheitsgründen. Aktuelle Infos
							...
							<Card>
								<CardContent className="pt-6 pb-6 px-6">
									...
									<p className="text-muted-foreground text-sm">
										...
										Kletterwald. Adresse: Etzenbach 1, 79219
										Staufen im Breisgau. Gut erreichbar über
										die B31.
									</p>
								</CardContent>
							</Card>
							<Card>
								<CardContent className="pt-6 pb-6 px-6">
									...
									<p className="text-muted-foreground text-sm">
										Zuglinie S3, Haltestelle „Etzenbach".
										Von Staufen Bahnhof ca. 10 Minuten
										Fahrt. Fahrpläne unter VAG Freiburg.
									</p>
								</CardContent>
							</Card>
						</div>
				...
				<section className="py-16 bg-secondary/50">
					<div className="container mx-auto px-4 text-center">
						...
						<p className="text-muted-foreground mb-8 max-w-lg mx-auto">
							Reserviere vorab und sichere dir deinen Platz –
							besonders an Wochenenden und Feiertagen empfohlen!
						</p>
